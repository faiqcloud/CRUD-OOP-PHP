...
<html lang="en">
  ...
  <body>
    <div class="container mt-3">
        ...
        <hr class = "mt-0">
        <!-- form edit data -->
        <form method = "POST" action = "prosses.php?aksi=update">
            <?php
            ...
            <?php $vkelamin = $dataUser['jenis_kelamin'];?>
            <select class="form-select" aria-label="Default select example" name = "jnskelamin" id = "jnskelamin">
                <option selected value="0" <?php echo ($vkelamin=="0") ? "selected" : "" ?>>pilih jenis kelamin</option>
                <option value="Laki-laki" <?php echo ($vkelamin=="Laki-laki") ? "selected" : "" ?>>Laki-laki</option>
                <option value="Perempuan" <?php echo ($vkelamin=="Perempuan") ? "selected" : "" ?>>Perempuan</option>
            </select>
            </div>
            <div class="mb-3">
                <label for="foto" class="form-label">Tambahkan Foto Anda</label>
                <input <?php if(!isset($_GET['foto']))  ?> class="form-control" type="file" id="foto" name="foto" accept="image/*">
            </div>
                <?php
                    }
                ?>
